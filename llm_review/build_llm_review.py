...
def main():
    ...
    # Per-confidence-variant node exposures (conf_min 1/2/3) for the
    # dashboard's mapping-confidence selector.
    by_conf = {}
    for conf_min in (1, 2, 3):
        ...
        for name, info in node_dict.items():
            if name in PARENTS:
                members = [sk for ch in HIERARCHY[name] for sk in node_dict[ch]["skills"]
                           if node_dict[ch]["conf"].get(sk, 3) >= conf_min]
            else:
                members = [sk for sk in info["skills"]
                           if info["conf"].get(sk, 3) >= conf_min]
            vals = [entries[sk][2] for sk in members]
            if vals:
                variant[name] = round(sum(vals) / len(vals), 3)
        by_conf[str(conf_min)] = variant
    ...
    with open(os.path.join(ROOT, "node_exposure.js"), "w", encoding="utf-8") as f:
        ...
        f.write("// per mapping-confidence variant (conf_min 1 = all, 2 = c2+, 3 = c3 only).\n")
        f.write("const NODE_AI_EXPOSURE_BY_CONF = ")
        json.dump(by_conf, f, ensure_ascii=False, separators=(",", ":"))
        f.write(";\nlet NODE_AI_EXPOSURE = NODE_AI_EXPOSURE_BY_CONF[\"1\"];\n")
